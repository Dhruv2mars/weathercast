...
The PostgreSQL serving migration runs idempotently at API or precision-ingestion startup. Forecast issues, raw MRMS/METAR source bytes, radar-frame metadata, and readiness observations are append-only. Concurrent writers for the same forecast ID return the canonical first issue. Set the same `ARCHIVE_MODE=postgres` and `DATABASE_URL` for `api:ingest-mrms` and `api:ingest-metar`; valid source-plus-derived batches commit atomically. Malformed METAR bytes are retained alone for diagnosis. Scientific study and calibration jobs still use the SQLite evidence ledger and must be migrated separately before horizontally scaling those workers. Raw payloads also require migration to content-addressed object storage before long-term global retention.
