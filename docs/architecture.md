...
The service separates liveness from readiness. Readiness proves the archive is writable, validates a contiguous fresh MRMS input sequence, and requires a configurable breadth of fresh verified METAR stations. Production configuration cannot disable these precision-data checks. Failures return only component-level pass/fail states and remove the instance from traffic without exposing source timestamps.
