...
The archive also freezes prospective radar-study definitions, coordinates, cadence slots, source-frame links, versioned evidence reports, calibration plans, isotonic artifacts, and evaluation bindings. Reports score only full-cohort issues against one deterministic verified METAR observation per run and horizon. Publication eligibility requires the study to end, at least 95% complete issuance, and every registered sample gate.
