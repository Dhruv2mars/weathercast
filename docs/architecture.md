...
The HTTP serving hot path and MRMS/METAR ingestion support pooled PostgreSQL in production, with atomic idempotent forecast writes, atomic valid source-plus-derived ingestion, immutable records, indexed freshness reads, and a real PostgreSQL CI test. Malformed METAR bytes remain archived for diagnosis without producing truth rows. SQLite remains the local default and the scientific study/calibration ledger. Global operation still requires porting radar inference and evidence workers to shared persistence, PostGIS, content-addressed raw-input object storage, licensed radar/satellite/station feeds, provider freshness alarms, distributed cache/rate limiting, and region-aware deployment. No production Precision tier or superiority claim is permitted until those inputs, an independent calibration holdout, and a lawful paired comparison pass their respective gates.
